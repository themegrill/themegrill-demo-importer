...
(function ($) {
	...
	demos.Collection = Backbone.Collection.extend({
		...
		search: function (term) {
			...
			results = this.filter(function (data) {
				name = data.get("name").replace(/(<([^>]+)>)/gi, "");
				description = data
					.get("description")
					.replace(/(<([^>]+)>)/gi, "");
				author = data.get("author").replace(/(<([^>]+)>)/gi, "");
	...
	demos.view.Demo = wp.Backbone.View.extend({
		...
		addFocus: function () {
			var $demoToFocus = $(":focus").hasClass("theme")
				? $(":focus")
				: $(":focus").parents(".theme");
		...
		preview: function (event) {
			...
			if (
				$(event.target)
					.not(".install-demo-preview")
					.parents(".theme-actions").length
			) {
				return;
			}
			...
			this.listenTo(preview, "demo:next", function () {
				...
				// Get next demo model.
				self.current = self.model.collection.at(
					self.model.collection.indexOf(current) + 1
				);
		...
		setNavButtonsState: function () {
			...
			if (0 === this.model.collection.indexOf(current)) {
				previousDemoButton.addClass("disabled").prop("disabled", true);
			...
			if (
				_.isUndefined(
					this.model.collection.at(
						this.model.collection.indexOf(current) + 1
					)
				)
			) {
				nextDemoButton.addClass("disabled").prop("disabled", true);
		...
		importDemo: function (event) {
			...
			$.confirm({
				title: "",
				content:
					'<div class="demo-import-confirm-message">' +
					(_this.model.get("ceAddonNotice")
						? wp.demos.data.settings.ceAddonNotice
						: "") +
					wp.demos.data.settings.confirmImport +
					"</div>",
				boxWidth: "50%",
				useBootstrap: false,
				buttons: {
					confirm: {
						...
						keys: ["enter"],
						btnClass: _this.model.get("ceAddonNotice")
							? "disabled demo-import-confirm-button"
	...
	demos.view.Preview = wp.Backbone.View.extend({
		...
		render: function () {
			...
			demos.router.navigate(
				demos.router.baseUrl(
					demos.router.demoPath + this.model.get("id")
				),
				{ replace: false }
			);
		...
		close: function () {
			this.$el
				.fadeOut(200, function () {
					$("body").removeClass(
						"demo-importer-active full-overlay-active"
					);
			...
			if (demos.router.selectedTab) {
				demos.router.navigate(
					demos.router.baseUrl("&browse=" + demos.router.selectedTab)
				);
			} else {
		...
		collapse: function (event) {
			...
			if ("true" === $button.attr("aria-expanded")) {
				$button.attr({
					"aria-expanded": "false",
					"aria-label": l10n.expandSidebar,
				});
			} else {
				$button.attr({
					"aria-expanded": "true",
					"aria-label": l10n.collapseSidebar,
				});
			}
		...
		processImport: function (
			...
			$(".theme-install-overlay")
				.find(".next-theme, .previous-theme")
				.addClass("disabled");
			...
			$(document).on(
				...
				function (event, response) {
					...
					$(".plugins-details .plugins-info").after(
	...
	demos.view.Demos = wp.Backbone.View.extend({
		...
		render: function () {
			...
			this.liveDemoCount = this.collection.count
		...
		renderDemos: function (page) {
			...
			if (demos.isNew && demos.data.settings.suggestURI) {
				this.$el.append(
					'<div class="theme add-new-theme"><a href="' +
						demos.data.settings.suggestURI +
						'" target="blank"><div class="theme-screenshot"><span></span></div><h2 class="theme-name">' +
						l10n.suggestNew +
						"</h2></a></div>"
				);
			}
	...
	demos.view.Search = wp.Backbone.View.extend({
		...
		doSearch: function (event) {
			...
			if (event.target.value) {
				demos.router.navigate(
					demos.router.baseUrl(
						demos.router.searchPath + event.target.value
					),
					options
				);
			} else {
		...
		pushState: function (event) {
			...
			if (event.target.value) {
				url = demos.router.baseUrl(
					demos.router.searchPath +
	...
	demos.view.InstallerSearch = demos.view.Search.extend({
		...
		doSearch: function (value) {
			...
			demos.router.navigate(
				demos.router.baseUrl(
					demos.router.searchPath + encodeURIComponent(value)
				),
				{ replace: true }
			);
		},
	...
	demos.view.Installer = demos.view.Appearance.extend({
		...
		render: function () {
			...
			this.listenTo(this, "demo:end", function () {
				...
				_.extend(self.collection.currentQuery.request, {
					page: self.collection.currentQuery.page,
				});
			...
			this.listenTo(this.collection, "query:fail", function () {
				...
				$(".theme-browser")
					...
					.before(
						'<div class="error"><p>' +
							l10n.error +
							'</p><p><button class="button try-again">' +
							l10n.tryAgain +
							"</button></p></div>"
					);
		...
		onSort: function (event) {
			...
			demos.router.navigate(
				demos.router.baseUrl(demos.router.browsePath + sort)
			);
		},
	...
	demos.RunInstaller = {
		init: function () {
			...
			this.view.SearchView.doSearch = _.debounce(
				this.view.SearchView.doSearch,
				500
			);
		},
		...
		routes: function () {
			...
			demos.router.on("route:preview", function (slug) {
				...
				if (self.view.view.demo && self.view.view.demo.preview) {
					self.view.view.demo.model = self.view.collection.findWhere({
						slug: slug,
					});
			...
			demos.router.on("route:sort", function (sort) {
				var type = demos.router.selectedType
					? demos.router.selectedType
					: $(".filter-links.pagebuilders li")
							.first()
							.find("a")
							.data("type");
				...
				if (!sort || !$('[data-sort="' + sort + '"]').length) {
					sort = "all";
					demos.router.navigate(demos.router.baseUrl("&browse=all"), {
						replace: true,
					});
				}
