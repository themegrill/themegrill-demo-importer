...
import { DemoType, PagebuilderCategory, TDIDashboardType } from './lib/types';
...
const Home = () => {
	...
	const [pagebuilders, setPagebuilders] = useState<PagebuilderCategory[]>([]);
	...
	const loading = useMemo(() => {
		return (
			!localizedData || demos.length === 0 || pagebuilders.length === 0 || categories.length === 0
		);
	}, [localizedData, demos, pagebuilders, categories]);

	useEffect(() => {
		if (localizedData?.data) {
			setDemos(localizedData.data.demos || []);
			setPagebuilders(localizedData.data.pagebuilders || []);
