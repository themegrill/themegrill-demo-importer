...
const Home = () => {
	...
	// Get current pagebuilder display text
	...
	// Reset pagebuilder if it becomes invalid
	useEffect(() => {
		if (!currentPagebuilder && pagebuilders.length > 0) {
			setPagebuilder('all');
		}
	}, [currentPagebuilder, pagebuilders, setPagebuilder]);
	...
	// Reset category when theme or pagebuilder changes
	useEffect(() => {
		setCategory('all');
	}, [theme, pagebuilder]);
